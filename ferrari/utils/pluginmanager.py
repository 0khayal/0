...
from ferrari import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"ferrari/plugins/{shortname}.py")
        checkplugins(path)
        name = f"ferrari.plugins.{shortname}"
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"ferrari.plugins.{shortname}"] = mod
        LOGS.info(f"تم تحميل ملف {shortname}")
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    ferrari.remove_event_handler(i)
    ...
    try:
        name = f"ferrari.plugins.{shortname}"
        for i in reversed(range(len(ferrari._event_builders))):
            ev, cb = ferrari._event_builders[i]
            if cb.__module__ == name:
                del ferrari._event_builders[i]
